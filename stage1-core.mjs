...
function blockedReason(a,s){
 ...
 if(a.eventId&&!eligible(a,s))return String(evaluate(a.unavailableReason,s,'That response is not available now.'));
...
function directedOpportunity(s,pool){
 ...
 const house=['repair_sun_promise','repair_fool_trust','meet_priestess','meet_fool','meet_sun','visit_brothel','company_sun','company_fool','company_priestess','talk_madame'];
...
function houseDescription(s){
 ...
 if(s.flags.metPriestess)lines.push('A reading with THE PRIESTESS costs $8.');
 if(s.flags.metSun)lines.push('Supper with THE SUN costs $6.');
 if(s.flags.cancelledForSun)lines.push('THE SUN wants you to answer for the appointment you cancelled. Private company must wait.');
 if(s.flags.betrayedFoolName||s.flags.blamedFool)lines.push('THE FOOL is keeping her distance. You can look for a chance to apologize.');
 return lines.join('\n');
}
