...
function advanceTime(s,changes,duration,source){const end=round(s.hours+duration);while(s.hours<end-.00001){const boundary=(Math.floor(s.hours/24)+1)*24;const dt=Math.min(1,end-s.hours,boundary-s.hours);const loss=s.stats.disquiet*dt/24;const prior=s.stats.rapture;appendChange(s,changes,'rapture',-loss,source,'Disquiet through the day');s.dayLedger.disquietLoss=round(s.dayLedger.disquietLoss+Math.min(prior,loss));appendChange(s,changes,'hunger',metaphysicalRate(s)*dt,source,'Metaphysical Hunger grows with time');s.hours=round(s.hours+dt);automaticCare(s,changes);maybeReset(s,changes,source);if(Math.abs(s.hours-boundary)<.0001){if(s.rhythm.mode==='spaced'){appendChange(s,changes,'disquiet',-nightlyDisquietRelief(s),'night','Sleep still comes between the days');appendChange(s,changes,'rapture',emotionalEffect(s,{category:'night'},'rapture',10+Math.min(4,s.lifestyle/20)),'night','Rest in the days between');}if(s.flags.modelLaunched){const profit=44+(s.dayLedger.day*7%13);appendChange(s,changes,'money',profit,'trader','Your tested automated trader');s.dayLedger.income+=profit;repayDebt(s,changes,profit,'trader');}pay(s,changes,6,'night','Daily living costs');if(s.dayLedger.day%7===0)pay(s,changes,65,'night','Weekly rent and bills');if(s.flags.careContract&&hasHelper(s)&&s.dayLedger.deliveries>0)pay(s,changes,3,'night','Your helper’s transport');const report={...s.dayLedger,hungerEnd:s.stats.hunger,resets:s.resets-s.dayLedger.resetsStart};s.nights.push(report);s.nights=s.nights.slice(-90);s.dayLedger=ledger(s);}}}
...
function resolve(state,a,{preview=false}={}){
 const s=clone(state),changes=[];const succeeded=!a.task||preview||random(s)<taskSuccessChance(state,a);const effects=evaluate(succeeded?a.effects:a.task.failureEffects,state,{});const duration=durationFor(state);
 ...
 maybeReset(s,changes,a.id);const priorNightCount=s.nights.length;advanceTime(s,changes,duration,a.id);
 ...
 if(a.id==='night_rest'||state.rhythm.mode==='spaced'){
  const interval=s.nights.filter(n=>n.day>=dayNumber(state)&&n.day<dayNumber(s));const report=interval.length?{...interval.at(-1),days:interval.length,disquietLoss:interval.reduce((n,r)=>n+r.disquietLoss,0),expenses:interval.reduce((n,r)=>n+r.expenses,0),income:interval.reduce((n,r)=>n+r.income,0),deliveries:interval.reduce((n,r)=>n+r.deliveries,0),bags:interval.reduce((n,r)=>n+r.bags,0),resets:interval.reduce((n,r)=>n+r.resets,0)}:s.nights.at(-1);let dream='no dream you can remember.';const candidates=NIGHT_DREAMS.filter(d=>!d.when||d.when(s));if(candidates.length&&dayNumber(state)%(s.rhythm.mode==='spaced'?4:3)!==0)dream=candidates[(dayNumber(state)-1)%candidates.length].text;
...
function selectOffers(s){
 ...
 if(s.turn-s.lastEventTurn>=7&&s.flags.pactMade){const e=storyEvents.find(e=>!s.completedEvents.includes(e.id)&&eventFits(e,s));if(e){s.pendingEvent=e.id;s.offers=e.options.map(o=>o.id);if(/person_confrontation/.test(e.id)&&!s.flags.choiceDiscovered)queueScene(s,'choice_discovery','choice');return;}}
 ...
 if(s.flags.pactMade&&s.stats.hunger>=50&&!(s.flags.careContract&&hasHelper(s)&&!s.flags.deliveryMissed)){if(!add('feed_friend'))if(!add('provision_and_feed'))add('emergency_delivery');}
...
function offerView(a,s){const scheduled=a.id==='temporary_shift'&&isScheduledWork(s);const action=scheduled?{...a,requirement:()=>0}:a;const lockedReason=blockedReason(action,s);const effects=evaluate(a.effects,s,{});const cost=Number(effects.money||0);return {id:a.id,label:scheduled?'Work your scheduled grocery shift':a.label,description:(scheduled?'You are on the rota. 9 a.m.–5 p.m. $52 before bills. ':String(evaluate(a.description,s,'')))+(cost<0&&!/\$/.test(String(evaluate(a.description,s,'')))?` Costs $${-cost}.`:''),category:a.category||'other',duration:durationFor(s),requirement:currentRequirement(action,s),available:!lockedReason,lockedReason,blinking:!lockedReason&&Number(effects.rapture||0)>0,thorny:Number(effects.rapture||0)<0,preview:lockedReason?[]:resolve(s,action,{preview:true}).changes,finish:a.id===FINISH.id||a.id==='stage1_complete',uncertain:!!a.task,mandatory:scheduled||a.id==='night_rest'||a.id==='return_to_hole'&&!s.flags.pactMade&&s.stats.hunger>=65};}
