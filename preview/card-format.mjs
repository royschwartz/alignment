...
export class CardTransition {
 constructor({from,to,held,width,height,outgoing=[],incoming=[],selected,transfers=[],reduced=false,overlay=null}){
  Object.assign(this,{width,height,selected,reduced,overlay});this.duration=reduced?180:FORMAT_MS;
 ...
 frame(elapsed){
  ...
  if(this.reduced){this.overlay?.(g,elapsed/this.duration);return this.canvas;}
  ...
  for(const item of this.outgoing){
   ...
   if(card.action===this.selected||this.transfers.some(t=>t.card.action===card.action&&t.card.delta<0)){
    for(const p of points){const progress=Math.abs(p.y-card.h/2)<24?Math.max(m.dissolve,m.labelDissolve):m.dissolve;if(p.rank>=progress)g.fillRect(card.x+p.x,card.y+p.y,1,1);}
